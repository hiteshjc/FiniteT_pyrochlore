...
void make_J_mats(double J1,double J2,double J3,double J4, 
		 std::vector<RMatrix> &Jmats, std::string hamtype);

void make_g_mats(double gxy,double gz, std::vector<RMatrix> &gmats,std::string hamtype); 
...
class Spin_Half_Ross: public Ham
{
    ...
    void init(std::vector< std::vector<int> > &filepairs, std::vector<int> & sublattice, 
	      ...
	      std::vector<int> &t1, std::vector<int> &t2, std::vector<int> &t3, double k1, double k2, double k3, std::string hamtype)
    {
	this->bondtype.clear();
	this->pairs=pairs;
	...
	make_J_mats(J1,J2,J3,J4,this->Jmats,this->hamtype);
	make_g_mats(gxy,gz,this->gmats,this->hamtype);
    }
